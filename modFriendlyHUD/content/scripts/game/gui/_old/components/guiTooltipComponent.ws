...
class W3TooltipComponent 
{
	...
	public function GetBaseItemData( item : SItemUniqueId, itemInvComponent : CInventoryComponent, optional isShopItem : bool, optional compareWithItem : SItemUniqueId, optional compareItemInv : CInventoryComponent ) : CScriptedFlashObject
	{
		...
		//modFriendlyHUD begin: show recycling parts
		if(GetFHUDConfig().showRecyclingParts)
		{
		...
		//---=== modFriendlyHUD ===---
		if(GetFHUDConfig().showStashItemQuantity)
		{
			if(itemInvComponent == m_playerInv)
			{
				durabilityValue = wplayer.GetHorseManager().GetInventoryComponent().GetItemQuantityByName(itemName);
				if(durabilityValue > 0)
				{
					tmpStr = "(" + IntToString(durabilityValue) + " " + GetLocStringByKey("fhud_item_quant_in_stash") + ") " + tmpStr;
				}
			}
			else if(itemInvComponent == m_shopInv)
			{
				durabilityValue = m_playerInv.GetItemQuantityByName(itemName) + wplayer.GetHorseManager().GetInventoryComponent().GetItemQuantityByName(itemName);
				if(durabilityValue > 0)
				{
					tmpStr = "(" + IntToString(durabilityValue) + " " + GetLocStringByKey("fhud_item_quant_owned") + ") " + tmpStr;
				}
			}
		}
		//---=== modFriendlyHUD ===---
	...
	private function AddPartsList( item: SItemUniqueId, itemInvComponent : CInventoryComponent, out description : string )
	{
		...
		if( partList.Size() > 0 )
		{
			...
			description += "<br>" + GetLocStringByKeyExt("fhud_recycling_parts") + ":";
