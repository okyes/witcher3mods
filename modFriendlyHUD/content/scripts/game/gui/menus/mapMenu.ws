...
class CR4MapMenu extends CR4MenuBase
{
	...
	private var disabledPinsCache : array< string >; //modFriendlyHUD
	...
	event  OnConfigUI()
	{
		...
		if ( manager )
		{
			...
			disabledPinsCache = manager.GetDisabledMapPins(); //modFriendlyHUD
		}
	...
	event  OnClosingMenu()
	{
		...
		//modFriendlyHUD begin
		if( GetFHUDConfig().markersSyncWithMap )
		{
			disabledPins = theGame.GetCommonMapManager().GetDisabledMapPins();
			if( disabledPins.Size() != disabledPinsCache.Size() )
			{
				Mod3DMarkersSignalCache3DMarkers( false );
			}
			else
			{
				for( i = 0; i < disabledPins.Size(); i += 1 )
				{
					if( disabledPins[i] != disabledPinsCache[i] )
					{
						Mod3DMarkersSignalCache3DMarkers( false );
					}
				}
	...
	private function UpdateMapSettings()
	{
		...
		EnableUnlimitedZoom( GetFHUDConfig().mapUnlimitedZoom );
		//---=== modFriendlyHUD ===---
