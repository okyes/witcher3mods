	...
	private function PopulateData()
	{
		...
		{	
			...
			if (GetFHUDConfig().showItemCountWhenCrafting)
			{
				...
				if (!theGame.GetDefinitionsManager().IsItemSingletonItem(recipe.cookedItemName) && (playerItems + horseItems) > 0)
				{
					l_Title += " | " + playerItems;
					if(horseItems > 0)
						l_Title += "+" + horseItems + " |";
				...
				if (recipe.cookedItemQuantity > 1)
				{
					l_Title += " / +" + recipe.cookedItemQuantity + " /";
				}
			}
			//---=== modFriendlyHUD ===---
