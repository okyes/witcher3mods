...
class CR4InventoryMenu extends CR4MenuBase
{
	...
	event  OnConfigUI()
	{
		...
		{
			defaultTab = GetFHUDConfig().GetDefautltInventoryTab();
			_playerInv.SetFilterType( defaultTab );
	...
	public function ResetNewFlags()
	{
		...
		categoryNames = GetFHUDConfig().GetResetNewItemsInCategoriesArray();
		if(categoryNames.Size() > 0)
		{
			...
			{		
				...
				if ( uiData.isNew )
				{
					for( k = 0; k < categoryNames.Size(); k += 1 )
					{
						categoryName = categoryNames[k];
						if ( CheckIfShouldResetNewItemFlag( item, categoryName ) )
						{
							uiData.isNew = false;
	...
	function CheckIfShouldResetNewItemFlag( item : SItemUniqueId, categoryName : string ) : bool
	{
		var isJunk     		: bool;
		var isTrophy   		: bool;
		
		isTrophy = thePlayer.inv.IsItemTrophy( item );
		isJunk = ( !_playerInv.isItemReadable(item) && !_playerInv.isFoodItem(item) && ! _playerInv.isIngredientItem( item ) && !_playerInv.isWeaponItem( item ) && !_playerInv.isArmorItem( item ) && !_playerInv.isAlchemyItem( item ) && !_playerInv.isUpgradeItem( item ) && !_playerInv.isItemSchematic( item ) && !_playerInv.isToolItem(item) && !_playerInv.isHorseItem( item ) && !isTrophy );
		
		if ( categoryName == "Ingredients" )
		{
			return _playerInv.isIngredientItem( item ) && !_playerInv.IsItemDye( item );
		}
		else if ( categoryName == "Quest" )
		{
			return _playerInv.isQuestItem( item );
		}
		else if ( categoryName == "Others" )
		{
			return ( isJunk || _playerInv.isItemReadable( item ) ) && !_playerInv.isQuickslotItem( item ) && !isTrophy;
		}
		else if ( categoryName == "Edibles" )
		{
			return _playerInv.isFoodItem( item );
		}
		else if ( categoryName == "Roach" )
		{
			return _playerInv.isHorseItem( item );
		}
		else if ( categoryName == "Alchemy" )
		{
			return _playerInv.isAlchemyItem( item ) && !_playerInv.isFoodItem( item ) && !_playerInv.isItemUsable( item ) && !_playerInv.IsItemDye( item );
		}
		else if ( categoryName == "Equipment" )
		{
			return _playerInv.isWeaponItem( item ) || _playerInv.isArmorItem( item ) || _playerInv.isUpgradeItem( item ) || _playerInv.isToolItem( item ) || _playerInv.isItemUsable( item ) || _playerInv.isQuickslotItem( item ) || _playerInv.IsItemDye( item );
		}
		return false;
	}	
	//---=== modFriendlyHUD ===---
