...
class CR4ScriptedHud extends CR4Hud
{
	...
	//---=== modFriendlyHUD ===---
	private var m_onelinersModuleEnabled	: bool; 	default m_onelinersModuleEnabled	= true;
	//---=== modFriendlyHUD ===---
	...
	public function UpdateHudConfig(configName : name, updateHud : bool):void
	{
		...
		case 'OnelinersModule':
			{
				configValue = inGameConfigWrapper.GetVarValue('Hud', configName);
				m_onelinersModuleEnabled = ( configValue == "true" );
				module = (CR4HudModuleBase)(GetHudModule(NameToString(configName)));
				if (module)
				{
	...
	//---=== modFriendlyHUD ===---
	public function IsOnelinersModuleEnabled() : bool
	{
		return m_onelinersModuleEnabled;
	}
	
	public function SetEnabledTemporarily ( modName : string, toggle : bool, upd : bool, reason : string )
	{
		var module : CR4HudModuleBase;
		
		module = (CR4HudModuleBase)GetHudModule( modName );
		if ( module )
		{
			module.ToggleEnabledTemporarily( toggle, reason );
		}
		if ( upd )
		{
			UpdateHUD();
	...
	public function ToggleModulesTemporarily( modNames : array< string >, toggle : bool, reason : string )
	{
		var i : int;
		
		for( i = 0; i < modNames.Size(); i += 1 )
		{
			SetEnabledTemporarily( modNames[i], toggle, false, reason );
		}
		UpdateHUD();
	}
	//---=== modFriendlyHUD ===---
	...
	function OnQuestUpdate( journalQuest : CJournalQuest, isQuestUpdate : bool )
	{
		...
		Mod3DMarkersSignalCache3DMarkers( true ); //modFriendlyHUD
	}
	...
	{
		if( FactsQuerySum( "quest_map_pins_hack_disable_sound" ) > 0 ) //wghost81: disable update sound for FHUD and All Quest Objectives hacks
			return true;
