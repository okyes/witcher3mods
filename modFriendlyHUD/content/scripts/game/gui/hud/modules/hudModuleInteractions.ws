...
class CR4HudModuleInteractions extends CR4HudModuleBase
{
	...
	event OnTick( timeDelta : float )
	{
		...
		var showEntityIcon		: bool;
		var showEntityText		: bool;
		//---=== modFriendlyHUD ===---
			 ...
			 m_previousDisplayEntityDataRet != displayEntityDataRet )
		{
			...
			if ( _interactionEntity )
			{
				...
				//---=== modFriendlyHUD ===---
				showInteractionIcon = GetFHUDConfig().ShowInteractionIcon(_interactionEntityComponent.GetActionName());
				showInteractionText = GetFHUDConfig().ShowInteractionText(_interactionEntityComponent.GetActionName());
			...
			else if ( displayEntity )
			{
				...
				{
					//---=== modFriendlyHUD ===---
					showEntityIcon = GetFHUDConfig().ShowEntityIcon(actionName);
		...
		else
		{
			...
			if ( iconIndex > -1 )
			{
				if ( !m_focusInteractionIcons[ iconIndex ].m_isSeen )
				{
					...
					//---=== modFriendlyHUD ===---
					if( !GetFHUDConfig().disableAllInteractionPrompts && !GetFHUDConfig().hideWSInteractionIcons )
					{
