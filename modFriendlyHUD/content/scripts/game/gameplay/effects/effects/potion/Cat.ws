...
class W3Potion_Cat extends CBaseGameplayEffect
{
	...
	event OnUpdate( dt : float )
	{
		...
		//---=== modFriendlyHUD ===---
		if(!witcher) witcher = GetWitcherPlayer();
		if(GetFHUDConfig().disableCatWhenNotInDarkPlace)
		{
			if(witcher.IsInDarkPlace() || GetFHUDConfig().enableCatAtNight && GetDayPart(theGame.GetGameTime()) == EDP_Midnight)
			{
