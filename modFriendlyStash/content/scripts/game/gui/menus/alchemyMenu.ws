...
{	
	...
	{	
		...
		//modFriendlyStash begin
		bStashAccess = GetFriendlyStashConfig().IsAlchemyAccessAllowed(bHerbalist);
		m_alchemyManager.SetStashAccess(bStashAccess);
		//modFriendlyStash end
	...
	function GetItemQuantity( id : int ) : int //modFriendlyStash: count stash items for crafting
	{
		...
		{
			return GetWitcherPlayer().GetMutagenQuantityByNameForCrafting(itemsNames[id], bStashAccess);
		}
		else
		{
			return GetWitcherPlayer().GetItemQuantityByNameForCrafting(itemsNames[id], bStashAccess);
		}
	}
